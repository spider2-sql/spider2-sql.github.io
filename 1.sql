...
ranked_screens AS (
  SELECT
    *,
    LAG(page_category, 1) OVER (PARTITION BY user_pseudo_id, visitID ORDER BY event_timestamp_microseconds ASC) AS previous_page,
    LEAD(page_category, 1) OVER (PARTITION BY user_pseudo_id, visitID ORDER BY event_timestamp_microseconds ASC) AS next_page
  FROM 
...
PLPtoPDPTransitions AS (
  ...
  FROM
    ranked_screens
  WHERE
    page_category = 'PLP' AND next_page = 'PDP'
  GROUP BY page_title
), 
TotalPLPViews AS (
  SELECT
...
ConversionRates AS (
  SELECT
    p.page_title,
    (CAST(t.transitions AS FLOAT64) / p.total_plp_views) * 100.0 AS conversion_rate
  FROM
    TotalPLPViews p
  JOIN
    PLPtoPDPTransitions t
